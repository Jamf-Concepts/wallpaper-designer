...
class TokenDelegate: NSObject, URLSessionDelegate {
    ...
    func getToken(serverUrl: String, whichServer: String = "source", base64creds: String, completion: @escaping (_ authResult: (Int,String)) -> Void) {
        ...
        guard let _ = URL(string: "\(tokenUrlString)") else {
            ...
            WriteToLog.shared.message(theMessage: "[getToken] problem constructing the URL from \(tokenUrlString)")
...
//        print("[getToken]             token created: \(JamfProServer.tokenCreated)")
//        print("[getToken]         tokenAgeInSeconds: \(tokenAgeInSeconds)")
//        print("[getToken]          token exipres in: \(((JamfProServer.authExpires)*60)-tokenAgeInSeconds)")
//        print("[getToken] JamfProServer.currentCred: \(String(describing: JamfProServer.currentCred))")

        if !( JamfProServer.validToken && tokenAgeInSeconds < (JamfProServer.authExpires)*60 ) {
            WriteToLog.shared.message(theMessage: "[getToken] tokenAgeInSeconds: \(tokenAgeInSeconds)")
            WriteToLog.shared.message(theMessage: "[getToken] Attempting to retrieve token from \(String(describing: tokenUrl))")
//            print("[getToken] Attempting to retrieve token from \(tokenUrl ?? URL(fileURLWithPath: ""))")
                    ...
                    if httpSuccess.contains(httpResponse.statusCode) {
                        if let json = try? JSONSerialization.jsonObject(with: data!, options: .allowFragments) {
                            if let endpointJSON = json as? [String: Any] {
                                ...
                                WriteToLog.shared.message(theMessage: "[getToken] new token created for \(serverUrl)")
                                        ...
                                        if versionString != "" {
                                            ...
                                            if tmpArray.count > 2 {
                                                ...
                                                if ( JamfProServer.majorVersion >= 10 || (JamfProServer.majorVersion > 9 && JamfProServer.minorVersion > 34) ) {
                                                    JamfProServer.authType = "Bearer"
                                                    WriteToLog.shared.message(theMessage: "[JamfPro.getVersion] \(serverUrl) set to use OAuth")
                                                    
                                                } else {
                                                    JamfProServer.authType    = "Basic"
                                                    JamfProServer.accessToken = base64creds
                                                    WriteToLog.shared.message(theMessage: "[JamfPro.getVersion] \(serverUrl) set to use Basic")
                                                }
                                ...
                                WriteToLog.shared.message(theMessage: "[getToken] JSON error.\n\(String(describing: json))")
                            ...
                            WriteToLog.shared.message(theMessage: "[TokenDelegate.getToken] Failed to get an expected response from \(String(describing: serverUrl)).  Status Code: \(httpResponse.statusCode)")
                            JamfProServer.validToken = false
                            completion((httpResponse.statusCode, "failed"))
                            return
                        }
...
//                        _ = Alert.shared.display(header: "\(serverUrl)", message: "Failed to authenticate to \(serverUrl). \nStatus Code: \(httpResponse.statusCode)", secondButton: "")
                        WriteToLog.shared.message(theMessage: "[getToken] Failed to authenticate to \(serverUrl).  Response error: \(httpResponse.statusCode)")
                        JamfProServer.validToken = false
                        completion((httpResponse.statusCode, "failed"))
                        return
                    }
...
//                    _ = Alert.shared.display(header: "\(serverUrl)", message: "Failed to connect. \nUnknown error, verify url and port.", secondButton: "")
                    WriteToLog.shared.message(theMessage: "[getToken] token response error from \(serverUrl).  Verify url and port")
                    JamfProServer.validToken = false
                    completion((0, "failed"))
                    return
                }
...
//            WriteToLog.shared.message(theMessage: "[getToken] Use existing token from \(String(describing: tokenUrl))")
            completion((200, "success"))
            return
        }
    }
...
//            if LogLevel.debug { WriteToLog.shared.message(theMessage: "[Jpapi.action] skipping \(endpoint) endpoint with id \(id).") }
