...
//    @IBOutlet weak var header_TextField: NSTextField!
    ...
    @IBAction func selectServer_Action(_ sender: Any) {
        if selectedServer_ButtonCell.titleOfSelectedItem == "Add Server..." {
            ...
            let selectedServer = (availableServersDict[selectedServer_ButtonCell.title]?["server"]) as? String ?? ""
            if !selectedServer.isEmpty {
                jamfProServer_textfield.stringValue = selectedServer.baseUrl
                credentialsCheck()
    ...
    @IBAction func login_action(_ sender: Any) {
        ...
        if theSender == "Add" {
            JamfProServer.destination = jamfProServer_textfield.stringValue.baseUrl
            JamfProServer.username    = jamfProUsername_textfield.stringValue
            JamfProServer.password    = jamfProPassword_textfield.stringValue
            JamfProServer.displayName = displayName_TextField.stringValue
        } else {
            JamfProServer.displayName = selectServer_Button.titleOfSelectedItem!
        }
        ...
        if theSender == "Login" {
            ...
            let jamfUtf8Creds = "\(JamfProServer.username.lowercased()):\(JamfProServer.password)".data(using: String.Encoding.utf8)
...
//            print("[login_action]    userpass: \(JamfProServer.password.prefix(2))********")
            ...
            TokenDelegate.shared.getToken(serverUrl: JamfProServer.destination, whichServer: "source", base64creds: JamfProServer.base64Creds) { [self]
                ...
                if theResult == "success" {
                    DispatchQueue.main.async { [self] in
                        ...
                        if saveCreds_button.state == .on {
                            keychainItems.removeAll()
                            keychainItems.append(KeychainItem(service: JamfProServer.destination.fqdnFromUrl, account: JamfProServer.username, secret: JamfProServer.password))
                            let _ = Credentials.shared.save(arrayOfKeychainItems: keychainItems)
                        }
                                                
                        WriteToLog.shared.message(theMessage: "[ViewController] successfully authenticated to \(JamfProServer.destination)")
                        // save password if checked - end
                        
                        spinner_PI.stopAnimation(self)
                        delegate?.sendLoginComplete()
                        dismiss(self)
                    }
                } else {
                    WriteToLog.shared.message(theMessage: "[ViewController] authentication failed to \(JamfProServer.destination)")
                    _ = Alert.shared.display(header: "Attention:", message: "Failed to generate token for \n\(JamfProServer.destination) \nHTTP status code: \(statusCode)", secondButton: "")
                    spinner_PI.stopAnimation(self)
            ...
            login_Button.isEnabled = false
            if JamfProServer.destination.range(of: "^http", options: [.regularExpression, .caseInsensitive]) == nil {
...
//        fetchPassword()
    ...
    fileprivate func loginButtonState() {
    ...
    func controlTextDidEndEditing(_ obj: Notification) {
        if let textField = obj.object as? NSTextField {
            ...
            case "username":
                ...
                if jamfProPassword_textfield.stringValue.isEmpty {
                    jamfProPassword_textfield.stringValue = accountDict[jamfProUsername_textfield.stringValue.lowercased()] ?? ""
                }
            case "password":
                break
            default:
    ...
    func credentialsCheck() {
        ...
        if accountDict.count != 0 {
            for (username, password) in accountDict {
                ...
                if username == jamfProUsername_textfield.stringValue.lowercased() || accountDict.count == 1 {
                    jamfProUsername_textfield.stringValue = username
                    jamfProPassword_textfield.stringValue = password
                    break
                }
...
//        print("[viewDidLoad] sharedSettingsPlistUrl: \(sharedSettingsPlistUrl.path)")
        ...
        // clean out local file paths
        for (displayName, serverInfo) in availableServersDict {
            if (serverInfo["server"] as! String).prefix(1) == "/" {
                availableServersDict[displayName] = nil
            }
        }
...
//        print("[LoginVC.viewDidLoad] availableServersDict: \(availableServersDict)")
    ...
    override func viewDidAppear() {
        ...
        if jamfProServer_textfield.stringValue == "" {
            jamfProServer_textfield.becomeFirstResponder()
        } else if jamfProUsername_textfield.stringValue == "" {
            jamfProUsername_textfield.becomeFirstResponder()
        } else if jamfProPassword_textfield.stringValue == "" {
            jamfProPassword_textfield.becomeFirstResponder()
        }
    }
